...
prs_table[[pop]] = data.table(n = rep(rep(c(1:rep_num),each=length(sample2_list)), length(p_list)*length(rho_list)), 
                                     ...
                                     JointPRS_popcorn_auto_2 = rep(0,rep_num * length(p_list)*length(rho_list)*length(sample2_list)))
...
for (sample2 in sample2_list){
    ...
    # JointPRS_popcorn_2
    test_JointPRS_popcorn_2 = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/sim_result/JointPRS/sim",i,"_p",p,"_rho",rho,"/test_sim",i,"_p",p,"_rho",rho,"_",sample1,"_",sample2,"_JointPRS_popcorn_real_EUR_",pop,"_prs_",pop,".sscore"))
    test_JointPRS_popcorn_2 = test_JointPRS_popcorn_2[match(test_pheno_id,test_JointPRS_popcorn_2$IID),]
    ...
    test_data = data.table(pheno = scale(test_pheno$pheno),
                           ...
                           JointPRS_popcorn_auto_2 = scale(test_JointPRS_popcorn_2$SCORE1_AVG))
    colnames(test_data) = c("pheno","JointPRS_auto_5","SDPRX_auto_2","JointPRS_popcorn_auto_2")
    for (j in 2:4){
...
write.table(prs_table_auto,paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/sim_result/PRS/sim_PRS_real_auto_no_val_JointPRS_popcorn_r2.csv"),quote=F,sep='\t',row.names=F,col.names=T)
