...
auto_method = c("XPASS","SDPRX")
auto_method_color = c("#FF8C00","#fec44f")
...
# Data preprocessing
prs_table_auto = fread(paste0("sim_h20.1_PRS_real_auto_r2.csv"))
prs_table_tune = fread(paste0("sim_h20.1_PRS_update_real_tune_r2.csv"))
...
p_80K <- ggplot(long_table, aes(x = minor_val_sample, y = mean_r2, group = method, fill = method)) +
  ...
  labs(title = "Non-European Population Training Sample Size = 80,000",
       x = "Non-European Tuning Sample Size",
       y = "R²") +
  theme_classic() + 
...
# sim_h20.1_PRS_tune_bar_plot_80K_highp.png width 1350 height 800
