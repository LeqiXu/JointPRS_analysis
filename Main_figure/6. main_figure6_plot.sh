## Figure6: same_cohort_PRS_violin_plot.png 13cm width X 15.5cm height (600dpi)
library(data.table)
library(dplyr)
library(tidyr)
library(ggplot2)
...
my_theme <- theme(
    ...
    panel.spacing = unit(0.01, "npc"),  # Almost no space between facets
    plot.margin = margin(0.5, 1, 0, 1, "mm")  # Reduce overall plot margins
)
...
p_GLGC_EAS = ggplot(long_table_with_JointPRS[which(long_table_with_JointPRS$trait %in% GLGC_trait & long_table_with_JointPRS$pop == "EAS"),],
  ...
  geom_violin(trim = TRUE, position = position_dodge(width = 0.4), color = "white", adjust = 1.5, scale = "width") +
  geom_point(aes(group = pop), position = position_jitterdodge(jitter.width = 0, dodge.width = 0.4), color = "black", size = 0.03, alpha = 0.8) +
  stat_summary(fun = mean, geom = "crossbar", width = 0.5, position = position_dodge(width = 0.4), color = alpha("black", alpha = 0.8), size = 0.2) +
  geom_hline(yintercept = 0, linetype = "solid", color = alpha("darkred", alpha = 0.7), size = 0.6) +
...
combined_plot = ggarrange(combined_plot_GLGC,combined_plot_PAGE,combined_plot_BBJ,combined_binary_plot, ncol = 1, nrow = 4, heights = c(1, 0.5, 0.5,0.5), labels = c("a","b","c","d"),font.label = list(size = 7, face = "bold"))
print(combined_plot)

ggsave("same_cohort_PRS_violin_plot.png", plot = combined_plot, width = 13, height = 15.5, dpi = 600, units = "cm", bg = "white")
