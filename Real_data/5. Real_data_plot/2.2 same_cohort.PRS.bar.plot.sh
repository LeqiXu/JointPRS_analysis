...
p_GLGC <- ggplot(long_table[which(long_table$trait %in% GLGC_trait),], aes(x = trait, y = mean_r2, group = method, fill = method)) +
...
p_PAGE <- ggplot(long_table[which(long_table$trait %in% PAGE_trait),], aes(x = trait, y = mean_r2, group = method, fill = method)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  geom_text(aes(label = annotation), vjust = 0, position = position_dodge(width = 0.9), color = "red", fontface = "bold", size = 5) +  # Bold stars
  scale_fill_manual(values = all_method_color, name = "Auto method") +
  facet_grid(~ pop, scales = "free_x", space = "free_x") +
...
p_BBJ <- ggplot(long_table[which(long_table$trait %in% BBJ_trait),], aes(x = trait, y = mean_r2, group = method, fill = method)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  geom_text(aes(label = annotation), vjust = 0, position = position_dodge(width = 0.9), color = "red", fontface = "bold", size = 5) +  # Bold stars
  scale_fill_manual(values = all_method_color, name = "Auto method") +
  facet_grid(~ pop, scales = "free_x", space = "free_x") +
...
colnames(prs_binary_table) = c("pop", "trait", "JointPRS","SDPRX","XPASS","PRS-CSx","PROSPER","MUSSEL","BridgePRS")
...
long_binary_table$method = factor(long_binary_table$method, levels = c("JointPRS","XPASS","SDPRX","PRS-CSx","MUSSEL","PROSPER","BridgePRS"))
...
## Obtain the plot
p_Binary <- ggplot(long_binary_table[which(long_binary_table$trait %in% Binary_trait),], aes(x = trait, y = mean_auc, group = method, fill = method)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  geom_text(aes(label = annotation), vjust = 0, position = position_dodge(width = 0.9), color = "red", fontface = "bold", size = 5) +  # Bold stars
  scale_fill_manual(values = all_method_color, name = "Auto method") +
  facet_grid(~ pop, scales = "free_x", space = "free_x") +
...
# same_cohort_PRS_bar_plot.png width 1350 height 1600