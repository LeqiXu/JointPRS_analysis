...
if [[ ! -e "${path_result}/${trait}/PROSPER_EUR_EAS_AFR/after_ensemble_${pop2}/PROSPER_prs_file.txt" ]]; then
        sbatch <<EOT
#!/bin/bash
#SBATCH --partition=scavenge,day,week
#SBATCH --requeue
...
for trait in Height BMI SBP DBP PLT; do
for pop2 in EAS AFR; do
cp /gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/diff_cohort/PROSPER/${trait}/PROSPER_EUR_EAS_AFR/after_ensemble_${pop2}/PROSPER_prs_file.txt /gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/Final_weight/diff_cohort/PROSPER/${trait}_PROSPER_EUR_EAS_AFR_beta_${pop2}.txt
done
done
...
beta_df = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/Final_weight/diff_cohort/PROSPER/",trait,"_PROSPER_EUR_EAS_AFR_beta_",pop2,".txt"))
beta_df = beta_df[,c("rsid","a1","weight")]

write.table(beta_df, file=paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/Final_weight/diff_cohort/PROSPER/",trait,"_PROSPER_EUR_EAS_AFR_beta_",pop2,".txt"), 
